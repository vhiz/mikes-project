...
export default function List({ list }) {
  const { error, isLoading, data } = useQuery(["books", list.id], async () => {
    const res = await apiUrl.get(`book/${list.id}`);
    return res.data;
  });

  const navigate = useNavigate();
  return (
    <div className="list" onClick={() => navigate(`/book/${list.id}`)}>
      {isLoading ? (
        "Loading"
      ) : error ? (
        "error"
        ...
        <>
          ...
          <div className="details">
            ...
            <span>
              <b>Time:</b> {moment(list.createdAt).fromNow()}
            </span>
          </div>
        </>
      )}
