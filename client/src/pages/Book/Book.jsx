import { useMutation, useQuery, useQueryClient } from "@tanstack/react-query";
import "./book.scss";
import { apiUrl } from "../../axios";
import { Link, useParams } from "react-router-dom";
import ImageGenerator from "../../components/star/Star";
import { useContext, useState } from "react";
import { AuthContext } from "../../context/authContext";

export default function Book() {
  const { id } = useParams();
  const queryClient = useQueryClient();
  const { currentUser } = useContext(AuthContext);
  const [sucess, setSucess] = useState(false);
  const [errors, setError] = useState(false);

  const { error, isLoading, data } = useQuery(["book", id], async () => {
    const res = await apiUrl.get(`book/${id}`);
    return res.data;
  });

  const { data: user } = useQuery(["currentUser", currentUser], async () => {
    const res = await apiUrl.get(`auth/${currentUser?._id}`);
    return res.data;
  });

  const mutation = useMutation({
    mutationFn: () => {
      return apiUrl.post(`/book/${currentUser._id}`, { bookId: id });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["listbooks", "currentUser"] });
    },
  });

  const addFav = async () => {
    mutation.mutate();
    window.location.reload();
  };
  const mutationDelete = useMutation({
    mutationFn: () => {
      return apiUrl.delete(`/book/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["books", id] });
      setSucess(true);
    },
    onError: () => {
      setError(true);
    },
  });

  const deleteBook = async () => {
    mutationDelete.mutate();
  };

  return (
    <div className="option">
      <div className="book">
        {isLoading ? (
          <div className="loader">
            <img src="/loader.png" alt="" />
          </div>
        ) : error ? (
          <div className="loader">
            <img src="/error.png" alt="" />
          </div>
        ) : (
          <>
            <div className="imgContainer">
              <img
                src={
                  data.volumeInfo?.imageLinks?.thumbnail ||
                  "data:image/png;base64,"
                }
                alt=""
              />
              {data?.volumeInfo?.isLibrary && <span>libary</span>}
              {data.accessInfo.pdf.isAvailable === true ? (
                <button>
                  <Link to={data?.accessInfo?.pdf?.acsTokenLink}>
                    Download now
                  </Link>
                </button>
              ) : (
                <p>No pdf available</p>
              )}
            </div>
            <div className="details">
              <div className="bookDelete">
                <h1>{data.volumeInfo.title}</h1>
                {data?.volumeInfo?.isLibrary && (
                  <button onClick={deleteBook}>Delete</button>
                )}
              </div>
              <h3>
                <b>Authors:</b>{" "}
                {data.volumeInfo?.authors?.map((author) => author).join(", ") ||
                  "Unknown"}
                ,
              </h3>
              <p
                dangerouslySetInnerHTML={{
                  __html: data.volumeInfo?.description,
                }}
              ></p>
              <span>
                <b>Published:</b> {data.volumeInfo?.publishedDate}
              </span>
              <span>
                <b>Publisher:</b> {data.volumeInfo?.publisher} 2010
              </span>
              <span>
                <b>Pages:</b> {data.volumeInfo?.pageCount}
              </span>
              <span>
                <b>Rating:</b>{" "}
                <ImageGenerator numImages={data.volumeInfo?.averageRating} />
              </span>
              <span>
                <b>Categories:</b>{" "}
                {data.volumeInfo?.categories
                  ?.map((category) => category)
                  .join(", ") || "Unknown"}
                ,
              </span>
              <button>
                <Link to={`${data.volumeInfo?.previewLink}`}>Reader mode</Link>
              </button>
              <button
                className={user?.books?.includes(id) ? "active" : null}
                onClick={addFav}
              >
                {user?.books?.includes(id)
                  ? "Remove from Libary"
                  : " Add to libary"}
              </button>
            </div>
          </>
        )}
      </div>
      {sucess ? (
        <div className="sucess">
          <div className="sucessDetails">
            <div className="logo">✔️</div>
            <span>Book Deleted Sucessfully </span>
            <Link to={"/"}>Redirect to home </Link>
          </div>
        </div>
      ) : errors ? (
        <div className="error">
          <div className="errorDetails">
            <div className="logo">❌</div>
            <span>Failed to delete book</span>
            <p onClick={() => setError(false)}>Go back</p>
          </div>
        </div>
      ) : null}
    </div>
  );
}
